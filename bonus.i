...
int papsCollision(volatile int x1, volatile int y1, volatile int width1, volatile int height1, volatile int x2, volatile int y2, volatile int width2, volatile int height2);
# 4 "bonus.c" 2
# 1 "game.h" 1
int lifeCount;
int timer;
...
void drawBonus() {
 ...
 for(int i = 0; i < 2; i++) {
  int ind = 63;
  int col = 200 + i*16;
  shadowOAM[ind].attr0 = (145 & 0xFF) | (0<<13) | (0<<14);
  shadowOAM[ind].attr1 = (col & 0x1FF) | (1<<14);
  shadowOAM[ind].attr2 = ((24)*32+((i*2) + 26));
 }
 if(clock >= 10) {
...
void drawQuadLeo() {
 shadowOAM[leo.index].attr0 = (leo.row & 0xFF)| (0<<13) | (0<<14);
 shadowOAM[leo.index].attr1 = (leo.col & 0x1FF) | (2<<14);
 shadowOAM[leo.index].attr2 = ((leo.sprite*4)*32+(leo.anistate*4));
}
...
void drawSteve() {
 shadowOAM[steve.index].attr0 = (steve.row & 0xFF)| (0<<13) | (0<<14);
 shadowOAM[steve.index].attr1 = (steve.col & 0x1FF) | (2<<14);
 shadowOAM[steve.index].attr2 = ((steve.sprite*4)*32+(steve.anistate*4));
}
...
void updateQuadLeo() {
 leo.aniCounter++;
 if((!(~(oldButtons)&((1<<5))) && (~buttons & ((1<<5)))) && leo.sprite != 4) {
  leo.sprite = 5;
  leo.col += leo.cdel;
 }
 if((!(~(oldButtons)&((1<<4))) && (~buttons & ((1<<4)))) && leo.sprite != 5) {
  leo.sprite = 4;
  leo.col += leo.cdel;
 }
}
...
void updateRaceLeo() {
 if((!(~(oldButtons)&((1<<5))) && (~buttons & ((1<<5)))) && leo.sprite != 2) {
  leo.sprite = 3;
  leo.col += leo.cdel;
  hOff++;
 } else if((!(~(oldButtons)&((1<<4))) && (~buttons & ((1<<4)))) && leo.sprite != 3) {
  leo.sprite = 2;
  leo.col += leo.cdel;
  hOff++;
 } else {
...
void updateSteve() {
 ...
 if(((steve.col) >> 8) >= startCol) {
  steve.col = ((startCol) << 8);
  steve.cdel = 2;
 }
