...
int papsCollision(int row1,int col1, int width1, int height1, int row2, int col2, int width2, int height2) {
    int topAboveBottom = row1 <= row2 + height2 && row1 >= row2;
    int vertical = col1 < col2 + width2 && col2 < col1 + width1;
    int leoOnTop = row1 + height1 >= row2 && row1 + height1 <= row2 + height2;
    int horizon = row1 < row2 + height2 && row1 + height1 > row2;
    int leftPastRt = col1 <= col2 + width2 && col1 > col2;
    int rightPastLt = col1 + width1 >= col2 && col1 + width1 <= col2 + width2;

    if(topAboveBottom && vertical) return 2;
    if(leoOnTop && vertical) return 1;
    if(horizon && leftPastRt) return 3;
    if(horizon && rightPastLt) return 4;
    return 0;
}
